...
// ── "Three ways in" section (Core homepage §3, Fluz-style) ────────────────────
// One platform, three audiences on the warm-grey field. Beige cards with a
// distinct product widget each (shipping label / shipments dashboard / API
// window) that bleeds past the card edge. Cards let visitors self-select — no
// hard fork — and the third card is the Plus signal.
...
const heading: React.CSSProperties = { ...sans, fontFamily: "var(--font-bricolage), var(--font-manrope), system-ui, sans-serif", fontWeight: 800, letterSpacing: "-0.02em", lineHeight: 1.06, color: ds.navy };
    ...
    <div className="relative flex items-center justify-center w-full" style={{ height: 250 }}>
      <div
        ...
        style={{
          ...
          boxShadow: "0 28px 55px rgba(236,90,38,0.38)",
        }}
      >
...
function ShipmentsDashboard() {
  ...
  return (
    <div className="w-full" style={{ maxWidth: 336 }}>
      <div className="p-4" style={{ background: ds.white, borderRadius: 20, boxShadow: "0 30px 60px rgba(28,30,61,0.22)", border: `1px solid ${ds.border}` }}>
...
function ApiWindow() {
  return (
    <div className="w-full" style={{ maxWidth: 340 }}>
      <div className="overflow-hidden" style={{ background: ds.white, borderRadius: 18, boxShadow: "0 30px 60px rgba(28,30,61,0.22)", border: `1px solid ${ds.border}` }}>
        {/* window chrome */}
  ...
  const heading = (
    <div className="relative z-10">
      <div className="flex items-center gap-2">
        <h3 style={{ ...anton, color: ds.navy, fontSize: "clamp(1.35rem, 2.2vw, 1.7rem)" }}>{label}</h3>
      ...
      <p className="mt-1.5" style={{ ...sans, fontSize: 15, color: "rgba(28,30,61,0.62)" }}>{blurb}</p>
    </div>
  );
  return (
    <a
      href={href}
      className={`group relative flex flex-col p-7 md:p-8 transition-transform hover:-translate-y-1 ${bleedTop ? "" : "overflow-hidden"}`}
      style={{ background: CARD_BG, borderRadius: 28, minHeight: 470 }}
    >
      {!labelAtBottom && heading}
      <div className={`flex-1 flex items-center justify-center ${bleedTop ? "-mt-10" : "py-5"}`}>{children}</div>
      {labelAtBottom && heading}
    </a>
  );
}
...
const STATS: [string, string][] = [
  ["70%", "off walk-in carrier rates"],
  ["1,000+", "five-star customer reviews"],
  ["Every", "major carrier, one login"],
  ["US ↔ CA", "cross-border shipping"],
];

// ── Section ───────────────────────────────────────────────────────────────────

export default function ShipTimePlatformTrio({
    ...
    <section className="px-5 md:px-10 py-20 md:py-28" style={{ background }}>
      <div style={{ maxWidth: 1180, margin: "0 auto" }}>
        ...
        <div className="text-center mb-12 md:mb-16">
          <h2 className="mx-auto" style={{ ...heading, fontSize: "clamp(2.1rem, 5vw, 3.4rem)", maxWidth: 780 }}>
            One platform for everything you ship
          </h2>
          <p className="mt-5 mx-auto" style={{ ...sans, fontSize: 16, lineHeight: 1.55, color: "rgba(28,30,61,0.6)", maxWidth: 480 }}>
        ...
        {/* stats bar */}
        <div className="mt-14 md:mt-20 grid grid-cols-2 md:grid-cols-4">
          {STATS.map(([big, label], i) => (
            <div
              key={label}
              className="px-5 py-6 md:py-4 text-center md:text-left"
              style={{ borderLeft: i % 2 === 0 ? "none" : `1px solid rgba(28,30,61,0.12)` }}
            >
              <div style={{ ...anton, color: ds.navy, fontSize: "clamp(2.1rem, 4vw, 2.9rem)", lineHeight: 1 }}>{big}</div>
              <div className="mt-2" style={{ ...sans, fontSize: 13.5, color: "rgba(28,30,61,0.6)", lineHeight: 1.4 }}>{label}</div>
            </div>
          ))}
        </div>
      </div>
